...
//! modified for SERAPH by Matari Audio.
//!
//! Submodule overview:
...
pub async fn run(
    ...
    // Adapted from xai-org/grok-build at bb7f39d5858cbf5e00de639367f59debbdcb0138;
    // SERAPH owns authentication and model discovery behind the injected ACP backend.
    let external_backend = crate::acp::backend_installed();
    ...
    let refreshed_auth = if external_backend {
        None
    } else {
        tokio::time::timeout(
...
fn print_exit_resume_hint(info: &ExitInfo, max_width: usize, w: &mut impl Write) {
    ...
    if info.minimal {
        let _ = writeln!(w, "  seraph --minimal --resume {}", info.session_id);
    } else {
        let _ = writeln!(w, "  seraph --resume {}", info.session_id);
    }
}
