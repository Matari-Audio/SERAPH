...
//! modified for SERAPH by Matari Audio to accept an alternate ACP agent.
//!
...
/// Spawn an alternate in-process ACP agent while retaining Grok's exact client and event loop.
pub async fn spawn_custom_agent<A, F>(
    factory: F,
    cancel: &CancellationToken,
) -> Result<SpawnedAgent>
where
    A: agent_client_protocol::Agent + Send + 'static,
    F: FnOnce(AcpClientTx) -> Result<A> + Send + 'static,
{
    ...
    let handle = spawn_agent_thread_custom(factory, acp_agent, agent_cancel.clone()).await?;
    Ok(SpawnedAgent {
...
async fn spawn_agent_thread_custom<A, F>(
    factory: F,
    ...
    F: FnOnce(AcpClientTx) -> Result<A> + Send + 'static,
{
    ...
    Ok(thread::Builder::new()
        .name("seraph-acp-agent".into())
        .spawn(move || {
            let local = tokio::task::LocalSet::new();
            let result = local.block_on(&rt, async move {
                let agent = Rc::new(factory(channel.tx.clone())?);
                tokio::task::spawn_local(
                    AcpGatewayReceiver::new(channel.rx, agent)
                        .with_tracing(true)
                        .run(),
                );
