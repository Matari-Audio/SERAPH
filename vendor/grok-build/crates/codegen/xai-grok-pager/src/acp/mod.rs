...
//!
//! Adapted from xai-org/grok-build at bb7f39d5858cbf5e00de639367f59debbdcb0138;
//! modified for SERAPH by Matari Audio to support an injected ACP backend.
//!
...
type BackendFuture = std::pin::Pin<
    Box<dyn std::future::Future<Output = anyhow::Result<spawn::SpawnedAgent>> + Send>,
>;
type BackendSpawner = fn(tokio_util::sync::CancellationToken) -> BackendFuture;
static BACKEND_SPAWNER: std::sync::OnceLock<BackendSpawner> = std::sync::OnceLock::new();

pub fn install_backend(spawner: BackendSpawner) {
    let _ = BACKEND_SPAWNER.set(spawner);
}

pub fn backend_installed() -> bool {
    BACKEND_SPAWNER.get().is_some()
}
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    let spawned = if let Some(spawner) = BACKEND_SPAWNER.get() {
        spawner(cancel.child_token()).await?
    } else {
...
pub fn startup_auth_metadata(
    ...
    let needs_login = first_method
        .map(|m| backend_installed() || AuthMethodKind::from_id(m.id()).needs_interactive_login())
        .unwrap_or(false);
...
pub fn find_interactive_login_method(
    ...
    let interactive = auth_methods
        .iter()
        .find(|m| backend_installed() || AuthMethodKind::from_id(m.id()).needs_interactive_login());
